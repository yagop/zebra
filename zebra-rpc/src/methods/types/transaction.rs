...
///
/// Only present at `getblock` verbosity 3, matching Bitcoin Core's `getblock` verbosity 3.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize, Getters, new)]
pub struct Prevout {
...
impl TransactionObject {
    ...
    pub fn add_prevouts(
        ...
        // A non-coinbase transaction has one spent outpoint per input, in input order, so the
        // inputs and outpoints zip one-to-one.
        for (input, outpoint) in self.inputs.iter_mut().zip(tx.spent_outpoints()) {
            if let (Input::NonCoinbase { prevout, .. }, Some(utxo)) =
                (input, spent_utxos.get(&outpoint))
            {
                *prevout = Some(Box::new(Prevout::new(
                    utxo.from_coinbase,
                    utxo.height.0,
                    Zec::from(utxo.output.value).lossy_zec(),
                    utxo.output.value.zatoshis(),
...
mod tests {
    ...
    fn add_prevouts_fills_prevouts_and_fee() {
        ...
        assert_eq!(prevout.value_zat(), first_input_value.zatoshis());
